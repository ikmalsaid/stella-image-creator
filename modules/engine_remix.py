...
def remix(input_remix, prompt, model, style, type, quality):
    image_bytes = BytesIO()
    input_remix.save(image_bytes, format='JPEG')
    ...
    data = {'image': ('input_remix.jpg', image_bytes.getvalue(), 'image/jpeg')}
...
def queue_remix(a, b, c, d, e, f, g, progress=ui.Progress()):
    quantities = 2
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for i in range(quantities):
            future = executor.submit(lambda x: remix(a, b, c, d, e, f), i)
